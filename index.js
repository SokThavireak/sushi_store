...
app.post("/api/cart", async (req, res) => {
  ...
  if (typeof req.user.id === "string" && req.user.id.startsWith("env-")) {
    return res
      .status(403)
      .json({ error: "Super Admins cannot use the shopping cart." });
  }
  // --- FIX END ---
...
app.get("/checkout", checkAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const locRes = await pool.query(
      "SELECT * FROM locations WHERE status = 'Open'"
    );
...
app.get(
  ...
  checkAuthenticated,
  checkRole(["manager", "admin", "store_manager"]),
  async (req, res) => {
    try {
...
app.post(
  "/admin/orders/items/delete/:itemId",
  checkAuthenticated,
...
app.post(
  "/admin/orders/items/update/:itemId",
  checkAuthenticated,
  checkRole(["manager", "admin", "store_manager", "staff", "cashier"]),
  async (req, res) => {
    const client = await pool.connect();
...
app.post(
  ...
  async (req, res) => {
    try {
      const id = req.params.id;
      const { status, payment_method, pickup_location, table_number } =
        req.body;
    ...
    try {
      ...
      if (!locId) {
        return res.render("error", {
          message: "Error: No valid location found.",
          user: req.user,
        });
      }
      ...
      if (locRes.rows.length === 0)
        return res.send("Error: Location ID not found.");
      ...
      const masterRes = await pool.query(
        "SELECT * FROM stocks ORDER BY category, name ASC"
      );
  ...
  async (req, res) => {
    if (typeof req.user.id === "string" && req.user.id.startsWith("env-")) {
      return res
        ...
        .json({
          error:
            "Super Admins cannot submit stock counts. Please create a real Manager account.",
        });
    }
    const client = await pool.connect();
    try {
      ...
      if (["admin", "manager"].includes(req.user.role)) {
        if (location_id) {
          ...
          const firstLoc = await client.query(
            "SELECT id FROM locations ORDER BY id ASC LIMIT 1"
          );
  ...
  async (req, res) => {
    try {
      ...
      const logRes = await pool.query(
        "SELECT * FROM daily_inventory_logs WHERE id = $1",
        [logId]
      );
      ...
      if (!isAdmin) {
        ...
        if (diffMinutes > 5)
          return res
            .status(403)
            .json({
              error: "Time limit exceeded. Cannot delete after 5 minutes.",
            });
      }

      // 3. Delete Items first (Foreign Key) then Log
      await pool.query("DELETE FROM daily_inventory_items WHERE log_id = $1", [
        logId,
      ]);
      await pool.query("DELETE FROM daily_inventory_logs WHERE id = $1", [
        logId,
      ]);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (logRes.rows.length === 0)
        return res.redirect("/manager/daily-stock/history");
      const log = logRes.rows[0];
      ...
      if (!isAdmin && !isUnlocked && (!isOwner || diffMinutes > 5)) {
        return res.send(
          `<script>alert('Edit time limit expired. Ask a Manager to unlock this report.'); window.location.href='/manager/daily-stock/history';</script>`
        );
      }

      const itemsRes = await pool.query(
        "SELECT * FROM daily_inventory_items WHERE log_id = $1 ORDER BY category, item_name",
        [logId]
      );
...
// --- SUBMIT EDIT (UPDATE) ---
app.post(
  "/api/manager/daily-stock/update/:id",
  checkAuthenticated,
  async (req, res) => {
    const client = await pool.connect();
    try {
...
app.post(
  "/api/manager/daily-stock/toggle-lock/:id",
  checkAuthenticated,
        ...
        if (req.user.assigned_location_id) {
          ...
          if (userLoc.rows.length > 0) {
            queryConditions.push(
              `dil.location_name = $${queryParams.length + 1}`
            );
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "store_manager") {
        ...
        if (
          locRes.rows.length > 0 &&
          log.location_name !== locRes.rows[0].name
        ) {
          return res
            .status(403)
            .send("Access Denied: This log belongs to another store.");
        }
      }
...
app.get(
  "/manager/daily-stock/history/:id",
  checkAuthenticated,
  async (req, res) => {
    try {
      const logId = req.params.id;
...
app.get("/debug/stocks", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM stock_requests ORDER BY id DESC"
    );
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(err);
      res.send(
        `<script>alert('Error adding item'); window.location.href='/admin/stock/menu';</script>`
      );
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      await pool.query("UPDATE stock_requests SET status = $1 WHERE id = $2", [
        status,
        requestId,
      ]);
    ...
    try {
      ...
      try {
        const stockRes = await pool.query(
          "SELECT * FROM stocks ORDER BY category, name"
        );
...
app.post("/api/stock/create", checkAuthenticated, async (req, res) => {
  if (typeof req.user.id === "string" && req.user.id.startsWith("env-")) {
    return res
      ...
      .json({
        error:
          "Super Admins cannot submit requests. Please create a real Manager account.",
      });
  }
  try {
    ...
    try {
      ...
      if (role === "store_manager") {
        if (status === "Cancel Requested" && currentStatus === "Pending") {
          ...
          return res
            .status(403)
            .json({ error: "Invalid status change for Store Manager" });
        }
      }
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const allLocationsRes = await client.query(
        "SELECT * FROM locations ORDER BY name ASC"
      );

      if (req.user.role === "store_manager") {
        if (req.user.assigned_location_id) {
          const myLoc = allLocationsRes.rows.find(
            (l) => l.id === req.user.assigned_location_id
          );
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const allLocationsRes = await pool.query(
        "SELECT * FROM locations ORDER BY name ASC"
      );
      ...
      res.render("admin/reports.ejs", {
        ...
        stats: {
          ...
          avgOrderValue:
            completedCount > 0
              ? (grossSales / completedCount).toFixed(2)
              : "0.00",
...
app.post(
  "/admin/orders/handle-request/:id",
  checkAuthenticated,
  checkRole(["admin", "manager", "store_manager"]),
  async (req, res) => {
    const { action } = req.body;
    const orderId = req.params.id;
    let newStatus = "";
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (nameCheck.rows.length > 0) {
        return res.send(
          `<script>alert('Error: Name exists.'); window.location.href='/admin/inventory';</script>`
        );
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const finalLocation =
        role === "store_manager" || role === "staff" || role === "cashier"
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (checkResult.rows.length > 0) {
        return res.send(
          `<script>alert('Email already exists'); window.location.href='/admin/users';</script>`
        );
      }
      ...
      if (
        ...
        return res.send(
          `<script>alert('Error: Staff, Cashiers, and Store Managers must have an assigned location.'); window.location.href='/admin/users';</script>`
        );
      }
